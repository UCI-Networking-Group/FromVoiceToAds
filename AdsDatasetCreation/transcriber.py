from llm_utility import *
import sys
import os


def transcribe_audio_file(input_file_path, output_folder_path):
    # Transcribe
    input_file = os.path.basename(input_file_path)
    transcript_file_path = os.path.join(output_folder_path, f"{os.path.splitext(input_file)[0]}_transcript.txt")
    transcript = transcribe_audio(input_file_path)
    save_to_text_file(transcript, transcript_file_path)


def transcribe_audio_files_in_folder(input_folder_path, output_folder_path):
    # Check if input folder exists
    if not os.path.isdir(input_folder_path):
        print(f"Error: Input folder '{input_folder_path}' not found!")
        return
    # Create the output folder if it doesn't exist
    if not os.path.isdir(output_folder_path):
        os.makedirs(output_folder_path)

    # Transcribe all audio files in the input folder
    for file_name in os.listdir(input_folder_path):
        if file_name.lower().endswith('.mp3'):
            file_transcript = file_name[:-4] + '_transcript.txt'
            if os.path.isfile(os.path.join(output_folder_path, file_transcript)):
                print(f"Done: {file_name}")
            else:
                input_file_path = os.path.join(input_folder_path, file_name)
                transcribe_audio_file(input_file_path, output_folder_path)


if __name__ == "__main__":
    if len(sys.argv) != 3:
        script_name = os.path.basename(__file__)
        print(f"Usage: python {script_name} input_(file or folder)_path output_folder_path")
        sys.exit(1)

    input_path = sys.argv[1]
    output_path = sys.argv[2]

    if not os.path.isdir(output_path):
        print(f"Invalid or non-existing output folder: {output_path}")
        sys.exit(1)

    if os.path.isfile(input_path):
        transcribe_audio_file(input_path, output_path)
    elif os.path.isdir(input_path):
        transcribe_audio_files_in_folder(input_path, output_path)
    else:
        print(f"Invalid input path: {input_path}")
        sys.exit(1)
